...
- (rc07) Preliminary code for doing vertical merges to compensate for limitations in jxls3
- (rc06) new ageGroupInfo accessor for writing templates. `formattedRange` summarizes the body weight categories and subcategories for an age group.  Output can be, for example, `55 B` if all the athletes are in that case, or `55-64 A` if all athletes are `A`.  If the athletes are not all in the same A/B/C subcategory, they are enumerated: `64A, 71B`.
