...
public class MQTTInterceptHandlers {
	...
	public static class ConnectionListener extends AbstractInterceptHandler {
		...
		public void onConnect(InterceptConnectMessage msg) {
			try {
				...
				// best-effort: detect transport/protocol for this connect message via
				// reflection
				try {
					...
					try {
						...
						if (o != null) {
							String s = o.toString();
							if (s.toLowerCase().contains("websocket")
							        || s.contains(Config.getCurrent().getParamMqttWsPort())) {
								transport = "ws";
							} else {
								transport = "tcp";
							}
						...
						try {
							...
							if (o2 != null) {
								...
								if (s2.toLowerCase().contains("websocket")
								        || s2.contains(Config.getCurrent().getParamMqttWsPort())) {
									transport = "ws";
								} else {
									transport = "tcp";
								}
					...
					if (clientId != null) {
						connectionTransport.put(clientId, transport);
					}
					try {
						MQTTMonitor.notifyGlobalClientConnected(clientId);
					} catch (Throwable t) {
					}
				} catch (Throwable t) {
					try {
		...
		public void onDisconnect(InterceptDisconnectMessage msg) {
			try {
				...
				try {
					MQTTMonitor.notifyGlobalClientDisconnected(clientId);
				} catch (Throwable t) {
				}
			} catch (Throwable t) {
	...
	// best-effort transport mapping (clientId -> transport string, e.g. "tcp" or
	// "ws")
	...
	public static void notifyGlobalClientConnected(String clientId) {
		...
		try {
			// Ignore descriptor updates for server-originated connections (contain
			// "_owlcms_")
		...
		try {
			LoggerFactory.getLogger(MQTTInterceptHandlers.class).debug(
			        "MQTT client connected: clientId={} globalActiveCount={}", clientId, globalActiveClientIds.size());
		} catch (Throwable t) {
		}
	}
	...
	public static void putDescriptor(String clientId, String desc) {
		// Ignore descriptor updates for server-originated connections (contain
		// "_owlcms_")
		if ((clientId == null) || isConfigClientId(clientId) || isServerClientId(clientId)) {
			return;
		}
	...
	public static void putLastSeen(String clientId, long ts) {
		if ((clientId == null) || isConfigClientId(clientId)) {
			return;
		}
