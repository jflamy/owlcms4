...
public class ResultsContent extends AthleteGridContent implements HasDynamicTitle {
	...
	protected void setGroupSelectionListener() {
		...
		groupSelect.addValueChangeListener(e -> {
			...
			groupResults.getElement().setAttribute("download", "results"+(currentGroup != null ? "_"+currentGroup : "_all") +".xls"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
		});
	}
	...
	private boolean checkFOP() {
		...
		if (liftingFop != null) {
			Notification.show(getTranslation("Warning_GroupLifting")+liftingFop.getName()+getTranslation("CannotEditResults"), 3000, Position.MIDDLE); //$NON-NLS-1$ //$NON-NLS-2$
