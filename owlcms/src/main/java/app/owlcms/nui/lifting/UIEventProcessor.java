...
public interface UIEventProcessor {
	 ...
	 *
	 * Do nothing if the event originates from ourselves -- if we stop the clock on the timekeeper device, there is no need to obey the command to stop the
	 * clock on all the other devices, since we are the cause of that event.
	 *
	...
	public static void uiAccessIgnoreIfSelfOrigin(Component attachedComponent, EventBus uiEventBus, UIEvent e,
		...
		// for locking purposes, we want the UI associated with the component (we don't
		// know who owns
