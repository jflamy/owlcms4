...
public interface BreakDisplay {
	...
	public default String inferGroupName() {
		...
		return MessageFormat.format(TranslationProvider.translate("BreakDisplay.1"), groupName); //$NON-NLS-1$
	}

	public default String inferMessage(BreakType bt) {
		switch (bt) {
		case FIRST_CJ:
			return TranslationProvider.translate("BreakDisplay.2"); //$NON-NLS-1$
		case FIRST_SNATCH:
			return TranslationProvider.translate("BreakDisplay.3"); //$NON-NLS-1$
		case INTRODUCTION:
			return TranslationProvider.translate("BreakDisplay.4"); //$NON-NLS-1$
		case TECHNICAL:
			return TranslationProvider.translate("BreakDisplay.5"); //$NON-NLS-1$
		default:
			return ""; //$NON-NLS-1$
		}
	}
