...
//				recomputeLiftingOrder(); // now done as part of decision(e)
			...
			if (changingAthlete.equals(clockOwner)) {
				...
				if (curWeight != newWeight) {
					...
					if (Athlete.zeroIfInvalid(changingAthlete.getCurrentDeclaration()) == newWeight) {
						Notification.show(MessageFormat.format(TranslationProvider.getTranslation("FieldOfPlay.0"), changingAthlete, newWeight),5000,Position.TOP_START); //$NON-NLS-1$
					}
					return;
				}
			} else {
	...
	private void unexpectedEventInState(FOPEvent e, FOPState state) {
		...
		String text = MessageFormat.format(TranslationProvider.getTranslation("FieldOfPlay.1"),e.getClass().getSimpleName(),state); //$NON-NLS-1$
		logger.warn(TranslationProvider.getTranslation("FieldOfPlay.2"),e.getClass().getSimpleName(),state); //$NON-NLS-1$
