...
public final class WaveFileReader extends SunFileReader {
	...
	public AudioFileFormat getAudioFileFormat(InputStream stream) throws UnsupportedAudioFileException, IOException {
		// fix for 4489272: AudioSystem.getAudioFileFormat() fails for InputStream, but
		// works for URL
		AudioFileFormat aff = getFMT(stream, true);
		// the following is not strictly necessary - but was implemented like that in
		// 1.3.0 - 1.4.1
	...
	private AudioFileFormat getFMT(InputStream stream, boolean doReset)
		...
		// $$kk: if "data" chunk precedes "fmt" chunk we are hosed -- can this legally
		// happen?
		nread = 0;
		while (true) {
			try {
