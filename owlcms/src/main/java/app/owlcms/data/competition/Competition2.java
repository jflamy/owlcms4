...
public class Competition2 {
    ...
    private static Competition2 competition;

    final static private Logger logger = (Logger) LoggerFactory.getLogger(Competition2.class);
     ...
     * @return the current
     */
    public static Competition2 getCurrent() {
        if (competition == null) {
            //FIXME backup
    ...
    public static void setCurrent(Competition2 c) {
        competition = c;
    }
...
//    private byte[] protocolTemplate;
    ...
    public HashMap<String, Object> computeReportingInfo() {
    ...
    public HashMap<String, Object> computeReportingInfo(String ageGroupPrefix, AgeDivision ad) {
    ...
    public HashMap<String, Object> computeReportingInfo(List<PAthlete> athletes, boolean full,
    ...
    public boolean equals(Object obj) {
        ...
        Competition2 other = (Competition2) obj;
    ...
    public List<Athlete> getGlobalSinclairRanking(Gender gender) {
        return getGlobalRankingOrElseRecompute(gender == Gender.F ? "wSinclair" : "mSinclair");
    }
    ...
    public List<Athlete> getGlobalRankingOrElseRecompute(String listName) {
        logger.warn("getGlobalRankingOrElseRecompute {}", LoggerUtils.whereFrom());
        ...
        if (isRankingsInvalid() || athletes == null) {
            ...
            while (isRankingsInvalid()) { // could be made invalid again while we compute
                ...
                logger.warn("recomputing global and robi rankings");
                
                // recompute because an athlete has been saved (new weight requested, good/bad lift, etc.)
                List<Athlete> allAthletes = AthleteRepository.findAllByGroupAndWeighIn(null, true);
                globalRankings(allAthletes);
                if (listName.contains("Robi")) {
                    robiRankings(allAthletes);
                }
    ...
    public boolean isRankingsInvalid() {
    ...
    public void setRankingsInvalid(boolean invalid) {
    ...
    private void categoryRankings(List<PAthlete> athletes) {
        ...
        reportingBeans.put("competition", Competition2.getCurrent());
...
//        }
        ...
        // sort only, use ranks stored in database
        if (Competition2.getCurrent().isCustomScore()) {
    ...
    private void globalRankings(List<? extends Athlete> list) {
        List<Athlete> sortedAthletes;
        List<Athlete> sortedMen;
        List<Athlete> sortedWomen;
        
        sortedAthletes = AthleteSorter.resultsOrderCopy(list, Ranking.BW_SINCLAIR);
        ...
        // leave as is, because category boundaries are the same across IWF age groups.
        sortedAthletes = AthleteSorter.resultsOrderCopy(list, Ranking.CAT_SINCLAIR);
        ...
        sortedAthletes = AthleteSorter.resultsOrderCopy(list, Ranking.SMM);
    ...
    /**
     * Robi ranks are specific to an age group
     */
    private void robiRankings(List<Athlete> list) {
//        List<Athlete> sortedAthletes;
//
//        // this computes a ROBI based on the registration category
//        sortedAthletes = AthleteSorter.resultsOrderCopy(list, Ranking.ROBI);
//        doRobi(sortedAthletes, "");

        // what we need instead is the ROBI ranking per age group
        List<AgeDivision> allUsedAgeDivisions = AgeGroupRepository.allAgeDivisionsForAllAgeGroups();
        for (AgeDivision ad : allUsedAgeDivisions) {
            // for age group championships, we don't know whether U15 and U17 for example
            // will be ranked together or separately.
            // too many age groups in Masters for this to make any sense
            if (ad != AgeDivision.MASTERS) {
                // compute each age group
                List<String> allAgeGroups = AgeGroupRepository.findActiveAndUsed(ad);
                for (String suffix: allAgeGroups) {
                    logger.warn("---- {}",suffix);
                    if (!"JR".contentEquals(suffix)) continue;
                    List<PAthlete> paList = AgeGroupRepository.allPAthletesForAgeGroup(suffix);
                    doRobi(paList, suffix);
                }
            }
//            // also do the full age division
//            // this would overwrite the agegroup
//            List<PAthlete> paList = AgeGroupRepository.allPAthletesForAgeGroupAgeDivision(null, ad);
//            String suffix = ad.name();
//            logger.warn("==== {}",suffix);
//            doRobi(paList, suffix);
        }
    ...
    private void doRobi(List<? extends Athlete> paList, String suffix) {
        List<Athlete> sortedAthletes;
        List<Athlete> sortedMen;
        List<Athlete> sortedWomen;
        sortedAthletes = AthleteSorter.resultsOrderCopy(paList, Ranking.ROBI);
        AthleteSorter.assignOverallRanksAndPoints(sortedAthletes, Ranking.ROBI);
        sortedMen = new ArrayList<>(sortedAthletes.size());
        sortedWomen = new ArrayList<>(sortedAthletes.size());
        splitByGender(sortedAthletes, sortedMen, sortedWomen);
        reportingBeans.put("mRobi"+suffix, sortedMen);
        for (Athlete a : sortedMen) {
            logger.warn("*** {} {} {} : robi rank {}","mRobi"+suffix, a.getShortName(), a.getMainRankings(), a.getRobi(), a.getRobiRank());
        }
        reportingBeans.put("wRobi"+suffix, sortedWomen);
        reportingBeans.put("mwRobi"+suffix, sortedWomen);
    }
    ...
    private void doComputeReportingInfo(boolean full, List<PAthlete> athletes, String ageGroupPrefix,
        ...
        runInThread(() -> {
            ...
            // the category ranks are stored in participation objects
            // in the database for each category and updated on each lift/change.
            // we only need to retrieve and sort.
            categoryRankings(athletes);

            // global rankings like Sinclair are written to the athlete.
            // ask for one, will trigger recompute of all of them if needed.
            getGlobalRankingOrElseRecompute("mRobi");
            reportRobi(ad, ageGroupPrefix);
    ...
    private void reportRobi(AgeDivision ad, String ageGroupPrefix) {
        List<Athlete> sorted;
        if (ageGroupPrefix != null && !ageGroupPrefix.isBlank()) {
            sorted = getOrCreateBean("mRobi"+ageGroupPrefix);
            reportingBeans.put("mRobi", sorted);
            sorted = getOrCreateBean("wRobi"+ageGroupPrefix);
            reportingBeans.put("wRobi", sorted);
            sorted = getOrCreateBean("mwRobi"+ageGroupPrefix);
            reportingBeans.put("wRobi", sorted);
        } else if (ad != null){
            sorted = getOrCreateBean("mRobi"+ad.name());
            reportingBeans.put("mRobi", sorted);
            sorted = getOrCreateBean("wRobi"+ad.name());
            reportingBeans.put("wRobi", sorted);
            sorted = getOrCreateBean("mwRobi"+ad.name());
            reportingBeans.put("wRobi", sorted);
        }
    }
    ...
    private void doTeamRankings(List<PAthlete> athletes, String suffix, boolean singleAgeGroup) {
        ...
        if (singleAgeGroup) {
            // create beans without suffix for teams page
