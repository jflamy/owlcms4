...
public class JPAService {
	...
	public static void init(boolean inMemory, boolean reset) {
		if (getFactory() == null) {
			...
			// h2 2.0 signals a redundant constraint as an error (behaviour has changed from
			// 1.4
			// ignore these warnings if any are present.
			// we don't know what database is being used here, but the code is trivial and
			// used only on init.
	...
	private static Properties h2FileProperties(String schemaGeneration, String dbUrl, String userName,
		...
		// use an explicit path as this allows connecting to the H2 server running
		// embedded
		String url;
