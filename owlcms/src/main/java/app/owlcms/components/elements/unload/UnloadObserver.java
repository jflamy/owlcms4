 ...
 * <a href= "https://vaadin.com/forum/thread/17523194/unsaved-changes-detect-page-exit-or-reload">the code by Kaspar Scherrer and Stuart Robinson</a>. This
 * component will broadcast events on {@code beforeunload} event in the browser. If {@link #isQueryingOnUnload()} is {@code true}, before the event the user
 * will be prompted about leaving the page. However, there is no way to find out what the user selected. If {@link #isQueryingOnUnload()} is {@code false}, the
 * event on the server will be called just before the page is unloaded. Note that the component must be present in the DOM structure in the browser for the
 * event to be received on the server.
 *
...
public final class UnloadObserver extends LitTemplate {
	 ...
	 *
	 * @return {@code true} when unloading the document from browser window results in showing a browser-native confirmation dialog and notifying
	 *         {@link UnloadListener}s; {@code false} otherwise.
	 */
	 ...
	 *
	 * @param queryingOnUnload When {@code true}, {@link UnloadListener}s registered through {@link #addUnloadListener(UnloadListener)} will be notified and
	 *                         document unloading can be prevented. When {@code false}, nothing will happen when the document gets unloaded.
	 */
