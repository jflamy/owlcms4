 ...
 * code by Kaspar Scherrer and Stuart Robinson</a>. This component will
 * broadcast events on {@code beforeunload} event in the browser. If
 * {@link #isQueryingOnUnload()} is {@code true}, before the event the user will
 * be prompted about leaving the page. However, there is no way to find out what
 * the user selected. If {@link #isQueryingOnUnload()} is {@code false}, the
 * event on the server will be called just before the page is unloaded. Note
 * that the component must be present in the DOM structure in the browser for
 * the event to be received on the server.
 *
 * Warning: this class is a {@link UI}-scoped; the class is final, the
 * constructors are private and there is at most one instance per UI.
 *
...
public final class UnloadObserver extends PolymerTemplate<TemplateModel> {
	 ...
	 * the component receives {@code unload} event.
	 */
	 ...
	 *         in showing a browser-native confirmation dialog and notifying
	 ...
	 *                         through {@link #addUnloadListener(UnloadListener)}
	 *                         will be notified and document unloading can be
