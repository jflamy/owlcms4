"""
Generate JXLS3 TeamResults "Total Only" templates (A4 and Letter variants).

Identical to generate-team-results-template.py EXCEPT the points expressions use
the total-only getters:
  - team summary points:  team.totalOnlyPoints
  - member points:        member.totalOnlyPoints

These getters always return points derived from the TOTAL place ranking,
regardless of the championship's snatchCJTotalMedals (combined snatch+CJ+total)
setting. This produces a team-results sheet that ignores combined-medal scoring
and always uses total-only place points.

Three sheets per file: Men, Women, Mixed.
Post-processing in JXLSTeamResultsSheet.postProcess() removes sheets whose bean
list was absent (empty gender group) and hides the unused measure column.

Beans available (see generate-team-results-template.py for full description):
  mTeamItems / wTeamItems / mwTeamItems - List<TeamTreeItem>
  Each team TeamTreeItem: getName(), getTotalOnlyPoints(), getScore(),
                          getCounted(), getSize(), getCountedTeamMembers()
  Each member TeamTreeItem: getName(), getCategory(), getTotalOnlyPoints(),
                            getScore(), isDone()
...
VARIANTS = [
    ('TeamResults-TotalOnly-A4.xlsx', 9),
    ('TeamResults-TotalOnly-Letter.xlsx', 1),
]
...
for filename, paper_size in VARIANTS:
    ...
    def build_sheet(sheet_title, items_bean, gender_key, prefix):
        ...
        # Points: total-only getter; show only when showPoints is true; hide 0
        ws.write(2, 4,
                 '${' + showPts + ' && team.totalOnlyPoints != 0 ? team.totalOnlyPoints : ""}',
                 team_right_fmt)
        ...
        # Points: total-only getter; always show individual place points (hide 0)
        ws.write(4, 4,
                 '${member.totalOnlyPoints != 0 ? member.totalOnlyPoints : ""}',
                 data_right_fmt)
