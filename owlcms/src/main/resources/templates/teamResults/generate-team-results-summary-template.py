"""
Generate JXLS3 TeamResults "Summary" templates (A4 and Letter variants).

These templates show one row per team only:
  - no member-detail loop
  - no spacer rows between teams
  - same Men / Women / Mixed sheets and per-sheet visibility controls
...
VARIANTS = [
    ("TeamResults-Summary-A4.xlsx", 9),
    ("TeamResults-Summary-Letter.xlsx", 1),
]
...
for filename, paper_size in VARIANTS:
    ...
    def build_sheet(sheet_title, items_bean, gender_key, prefix):
        ...
        ws.write(3, 4,
                 '${' + show_points + ' && team.points != 0 ? team.points : ""}',
                 team_right_fmt)
        ws.write(3, 5,
    ...
    wb.close()
    print(f"Created {output} — 3 summary sheets (Men, Women, Mixed) via xlsxwriter")
