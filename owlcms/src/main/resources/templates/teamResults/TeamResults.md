...
Generated by the team-results template generator scripts in this directory.
...
Additional variants:
- `TeamResults-Summary-A4.xlsx` / `TeamResults-Summary-Letter.xlsx` — one row per team, no member details
- `TeamResults-TotalOnly-A4.xlsx` / `TeamResults-TotalOnly-Letter.xlsx` — full detail, but points always come from total rank only
- `TeamResults-TotalOnly-Summary-A4.xlsx` / `TeamResults-TotalOnly-Summary-Letter.xlsx` — summary-only and total-only combined
...
## Summary-only variant template

Create `TeamResults-Summary-A4.xlsx` / `TeamResults-Summary-Letter.xlsx` with:

| Row | Cols | Content | JXLS comment |
|-----|------|---------|--------------|
| 1 | A | `${competition.competitionName}` | `jx:area(lastCell="G4")` |
| 2 | A, D | Championship name; ageGroupPrefix + gender label | — |
| 3 | A–G | Summary headers only | — |
| 4 | A–G | Team summary row only | `jx:each(items="XXTeamItems" var="team" lastCell="G4")` |

Summary-only rules:
- no member-detail loop
- no spacer row between teams
- status header should use `${t.get("TeamResults.Status")}`
- points remain in column E and score remains in column G so the existing per-tab column hide logic still applies

## Total-only summary variant

Create `TeamResults-TotalOnly-Summary-A4.xlsx` / `TeamResults-TotalOnly-Summary-Letter.xlsx`
exactly like the summary-only template above, but use:
