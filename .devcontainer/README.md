...
- When the container finishes starting, choose the workspace file `owlcmsDevcontainer.code-workspace` from the workspace files list if prompted.
...
- If you see the launches missing, ensure the Java extensions are enabled for the workspace/profile and that you're using the `owlcmsDevcontainer.code-workspace` file.
...
- In Codespaces, the `onCreateCommand` and `postCreateCommand` are executed during provisioning. Choose the `owlcmsDevcontainer.code-workspace` workspace in the Codespaces workspace picker if prompted.
