...
public class FileServlet extends HttpServlet {
	 ...
	 * @param userPath    the untrusted path provided by the API user, expected to
	 *                    be relative to {@code baseDirPath}
	 */
	...
	private Path getFileFromPathInfo(HttpServletResponse response, String requestedFile)
		...
		try {
			...
			// object.
			// @webservlet processing takes care of preventing ../.. escaping so we don't
			// have to.
			...
			// logger.debug("relativeFileName = {} fixed = {} regex={}", relativeFileName,
			// unversionedName, target);
