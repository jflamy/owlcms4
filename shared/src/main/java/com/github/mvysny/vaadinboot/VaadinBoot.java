...
public class VaadinBoot {
	...
	/**
	 * The port where Jetty will listen for http:// traffic. Defaults to {@value #DEFAULT_PORT}. <br/>
	...
	/**
	 * Listen on interface handling given host name. Defaults to <code>null</code> which causes Jetty to listen on all interfaces. <br/>
	...
	/**
	 * The context root to run under. Defaults to `/`. Change this to e.g. /foo to host your app on a different context root <br/>
	 * Can be configured via the <code>SERVER_SERVLET_CONTEXT-PATH</code> environment variable, or <code>-Dserver.servlet.context-path=</code> Java system
	 * property.
	 */
	...
	/**
	 * If true, no classpath scanning is performed - no servlets nor weblisteners are detected. <br/>
	 * This will most probably cause Vaadin to not work and throw NullPointerException at <code>VaadinServlet.serveStaticOrWebJarRequest</code>. However, it's a
	 * good thing to disable this when starting your app with a QuickStart configuration.
	 */
	private boolean disableClasspathScanning = false;
	/**
	 * If true, the test classpath will also be scanned for annotations. Defaults to false. <br/>
	 * Only set to true if you have Vaadin routes in <code>src/test/java/</code> - it's a bit of an antipattern but quite common with Vaadin addons. See
	 * <a href="https://github.com/mvysny/vaadin-boot/issues/15">Issue #15</a> for more details. <br/>
	 * Ignored if {@link #disableClasspathScanning} is true.
	 */
	private boolean isScanTestClasspath = false;
	/**
	 * If true and we're running on JDK 21+, we'll configure Jetty to take advantage of virtual threads. <br/>
	 * Defaults to true.
	 */
	...
	/**
	 * Listen on network interface handling given host name. Pass in null to listen on all interfaces; pass in `127.0.0.1` or `localhost` to listen on localhost
	...
	/**
	 * When called, the test classpath will also be scanned for annotations. Defaults to false. <br/>
	 * Use only in case when you have Vaadin routes in <code>src/test/java/</code> - it's a bit of an antipattern but quite common with Vaadin addons. See
	 * <a href="https://github.com/mvysny/vaadin-boot/issues/15">Issue #15</a> for more details. <br/>
	 * Ignored if {@link #disableClasspathScanning} is true.
	 * 
	 * @return this
	 ...
	 * Defaults to true.
	 * 
	 * @param useVirtualThreadsIfAvailable if true (default), use virtual threads to handle http requests if running on JDK21+
	 * @return this
	 */
	...
	/**
	 * Runs your app. Blocks until the user presses Enter or CTRL+C. <br/>
	...
	/**
	 * Starts the Jetty server and your app. Blocks until the app is fully started, then resumes execution. Mostly used for testing.
	 * 
	 * @throws Exception when the webapp fails to start.
	 */
	...
	/**
	 * Invoked when the Jetty server has been started. By default, does nothing. You can for example dump the quickstart configuration here.
	 * 
	...
	/**
	 * Stops your app. Blocks until the webapp is fully stopped. Mostly used for tests. Never throws an exception.
	 * 
	...
	private void failToStart(Exception e) {
		try {
			if (server != null) {
				...
				if (message == null || message.contains("bind")) {
					...
					log.error("Probable cause: another instance is running, or another program is using the port");
				}
				server.stop(); // blocks until the webapp stops fully
				log.info("Stopped");
				server = null;
