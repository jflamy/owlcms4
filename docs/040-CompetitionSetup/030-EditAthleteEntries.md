...
These steps are performed from the `Prepare Competition` page![10](img/030-EditAthleteEntries/10a.png)

## Adding or Editing Athletes

In order to add or edit athletes, use the `Edit Athlete Entries` button.

- To add an athlete, use the `+` button at the top of the list.  If you have selected a session at the top, the athlete will be added to that session.![15](img/030-EditAthleteEntries/15.png)
...
![40](img/030-EditAthleteEntries/40a.png)
...
![50](img/030-EditAthleteEntries/50.png)

## Team Membership

Before [producing the Start List and Team Lists](./040-PreCompetitionDocuments), it is important to make sure that the team memberships are correct.  The `Team Membership` page makes it possible to see the team members for each team. 
...
![30](img/030-EditAthleteEntries/30.png)

### SBDE Loading Options

When loading an SBDE file, there are several options
![30](img/030-EditAthleteEntries/31.png)
...
- If the athlete is eligible to more than one category because qualifying totals were met, the additional categories are added are separated by a `;`. So a YTH athlete also eligible to both JR and SR would be `YTH M 81;JR M 81;SR M 81`  and if only eligible for JR, the string would be `YTH M 81;JR M 81` 
- Any category can be annotated with `/-T` if the athlete is not part of the corresponding team (by default, an athlete is included in the teams where he is eligible; if the `explicitTeams` feature switch is on, the default is reversed and members are marked with `/+T` instead -- see [Team Membership in the Registration File](./010-Registration#team-membership-in-the-registration-file)).
  In our example, if the athlete is eligible for SR but is NOT included in the SR team, then `/NoTeam` is added to the team from which the athlete is excluded.  In that case, the string will be `YTH M 81;JR M 81;SR M 81/-T`.  The annotation also works on the main category `YTH M 81/-T` is a legal specification.
- If the competition has mixed championships that require explicit participation, you will also see +MT as a marker.   -T,+MT would be legal to remove the athlete from the gendered team and add them to the mixed team. Any combination will be accepted (+T,-MT  or -MT,-T   etc.) to simplify mechanical processing.
