...
```powershell
curl -sfL https://github.com/owlcms/owlcms4/releases/download/4.27.0-alpha07/k3d_setup.yaml | envsubst | kubectl apply -f - 
```
...
- The latest release can always be reached using this format (note that the download part is inverted relative to the revision name for some unfathomable reason)
  ...
  ```bash
  curl -sfL https://github.com/${env.REPO_OWNER}/${env.O_REPO_NAME}/releases/latest/download/k3d_setup.yaml | envsubst | kubectl apply -f - 
  ```
