...
```powershell
curl -sfL https://github.com/owlcms/owlcms4/releases/download/4.23.1-SNAPSHOT/k3d_setup.yaml | envsubst | kubectl apply -f - 
```
...
- The latest release can always be reached using this format (note that the download part is inverted relative to the revision name for some unfathomable reason)
  ...
  ```bash
  curl -sfL https://github.com/jflamy/owlcms4/releases/latest/download/k3d_setup.yaml | envsubst | kubectl apply -f - 
  ```
