...
2. Setup different meetings in advance in Zoom for the weigh-ins and for the competition groups.  
   - If you anticipate that the schedule will be hard to keep, it may be a good idea to schedule individual meetings for each group, to avoid people coming in when a session has not ended.
3. Send the following directives to each participating team:
   1. A phone or laptop with Zoom is setup to show the platform.  The video should be in horizontal mode.
      - The camera showing the platform should ideally remain on, to make the video operator's life easier.
   2. When connecting, they should rename their video session to the LAST name of each athlete present (separated by "/" if more than one)
   3. They need a way to enter changes through Zoom chat.  This can be a second Zoom participant other than the video stream.  **All** declarations and changes will be **sent by chat**, to <u>Everyone</u>, using the athlete's **LAST name and the requested weight**.
   4. Finally, each team needs a way to see the public scoreboard through an internet browser.   Ideally, the scoreboard and the zoom chat should take place on a laptop separate from the video stream if the physical setup permits.
   5. Each team must be available on Zoom 15 minutes before the start of group, for the technical controller to go through all the video setups and make sure that athletes are visible.
   6. Each gym is responsible for making sure that they are ready when the camera switches to their location. 
      1. The bar should already be loaded with the correct weight
      2. The bar should be angled such that the loaded weight is visible to the camera
      3. Meet-specific rules can state that time starts immediately when the athlete is called, since having the proper weight loaded is entirely under the control of the team. 
4. Any Zoom session not used to broadcast a platform should turn off video -- all the chat-only sessions for weight changes should be video off.
   - The video operator can then right-click on a camera image and hide the non-video participants, leaving only the platforms.
