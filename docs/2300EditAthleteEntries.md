...
- Any category can be annotated with `/-T` if the athlete is not part of the corresponding team (by default, an athlete is included in the teams where he is eligible; if the `explicitTeams` feature switch is on, the default is reversed and members are marked with `/+T` instead -- see [Team Membership in the Registration File](2200Registration#team-membership-in-the-registration-file)).
