...
- `owlcms/fop/startup/A :status`: owlcms has completed the startup process.
    ...
    - DEPRECATED : the following message legacy message is equivalent : `owlcms/led/A :status`
- `owlcms/fop/config`  this message published in response to a `owlcms/config` request from the device.  Note that there can be multiple answers - one per platform.  The answers are all identical, all but the first one can be ignored.
...
- `owlcms/fop/resetDecisions/A :option`: The clock has started for a new attempt. All LEDs for referee and jury member decisions are turned off. 
...
- `owlcms/fop/decision/A :ref :decision`: 
...
- `owlcms/fop/juryMemberDecision/A :juryMember :decision`: 
...
- `owlcms/fop/decisionRequest/A :ref`: The third ref must make a decision.
    - LEGACY: `owlcms/decisionRequest/A/:ref :status` (status is `on` or `off`)
...
- `owlcms/fop/summon/A :official`: The jury summons an official to the jury table.
    - LEGACY: `owlcms/summon/A/:official :status` (status is `on` or `off`, off is unused, device turns it off on its own)
    - `official` is `1`, `2`, `3`
...
- `owlcms/fop/down/A`: The down signal has been given
    - device must turn itself off.
- `owlcms/fop/timeRemaining/A :seconds`
...
- `owlcms/fop/liftingOrderUpdated/A` Data entry has taken place (possibly changing requested weight or lifting order) or decision has been given.

- `owlcms/fop/ceremony/A :name :status`  A ceremony has started or stopped
...
- `owlcms/fop/break/A :breakType` A break has started.  There are no end break event.  THe `owlcms/fop/startlifting/A` message will indicate that competition has resumed. or a `ceremony` message will take place.  Not that JURY and CHALLENGE breaks are not be issued, they are issued as separate `juryDeliberation` events.
...
- `owlcms/fop/startLifting/A`   Lifting has started on the platform.  This is the normal way to end a break.
- `owlcms/fop/start/A :jsonAthlete :timeRemaining` Clock has started for athlete :jsonAthlete.  The json object contains 

  - `athleteName`  the full name of the athlete
  - `liftType` either `SNATCH` or `CLEANJERK`
  - `attemptNumber` 1 2 or 3

- `owlcms/fop/stop/A :timeRemaining` Clock has stopped for athlete :jsonAthlete. 
- `owlcms/fop/refereesDecision/A` `:decision` after a lift, when `good` or `bad` lift majority decisions are shown.  Used by observers to control lighting or other ambiance effects without having to parse the individual decisions to determine majority.  Issued at the same time as the individual decisions.

### Messages published by all devices

Only owlcms listens; all devices can query owlcms for its configuration

- `owlcms/config` request the owlcms configuration
  - no parameter
  - The response will come in an a `owlcms/fop/config` message.

### Messages published by the referee devices

Only owlcms listens; devices do not listen to one another.

- `owlcms/refbox/decision/A :ref :decision`: Ref decides the attempt is a good lift or no lift. 
    - `ref` is `1`, `2`, or `3`
    - `decision` is `good` or `bad`
    - DEPRECATED: This message is equivalent to `owlcms/decision/A :ref :decision`

- `owlcms/refbox/downEmitted/A`: The referee device has emitted a down signal.
...
- `owlcms/refbox/decision/A 0 :decision`: 
  - `ref` is `0` 
  - `decision` is `good` or `bad`
...
- `owlcms/clock/A :action`: The timekeeper has started, stopped, or reset the clock. `toggle` allows using the same button to start/stop the clock.
...
- `owlcms/jurybox/summon/A :official`: The jury has summoned an official.
    - `official` is `1`, `2`, `3`, `all` or `controller`
- `owlcms/jurybox/juryMember/decision/A :member :decision`: Jury member decides the attempt is a good lift or no lift.
    - `member` is `1`, `2`, `3`, `4`, or `5`
    - `decision` is `good` or `bad`
- `owlcms/jurybox/decision/A :decision`: The jury has made a decision.
    - `decision` is `good` or `bad`
- `owlcms/jurybox/break/A :breakEvent`: The jury has called for a break.
...
- General
    - When performing an action that may change the state of the field of play, publish the relevant message, then wait for the corresponding message to be published by owlcms before updating other devices. For example, when the jury summons a referee, publish `owlcms/jurybox/summon/A 1`, but do not play a sound from the referee control box until owlcms publishes `owlcms/fop/summon/A 1`.
...
- Referees
    - When a decision is made, publish `owlcms/refbox/decision/A 1 good`.
    - When a majority decision has been made, activate down signal.
        - Publish `owlcms/refbox/downEmitted/A`.
    - Reset decision tracking 3 seconds after all referees have made a decision.
        - Subscribe to `owlcms/fop/clockStart/A` for additional reset scenarios.
    - When summoned by the jury, play a sound.
        - Subscribe to `owlcms/fop/summon/A 1`.
- Down Signal
    - When a majority decision has been made, activate light and sound for 2 seconds.
        - Subscribe to `owlcms/fop/down/A` as a fallback; ignore if published within 5 seconds of on board signal.
- Jury
    - When the jury summons a referee, publish `owlcms/jurybox/summon/A 1`.
    - When a jury member has made a decision, publish `owlcms/jurybox/juryMember/decision/A 1 good`.
    ...
    - When the jury has reached an unanimous decision, publish `owlcms/jurybox/decision/A good`.
    - When the jury calls the controller, publish `owlcms/jurybox/summon/A controller`.
    - When the jury calls for a deliberation, publish `owlcms/jurybox/break/A deliberation`.
    - When the jury calls for a technical break, publish `owlcms/jurybox/break/A technical`.
    - When the jury ends a break, publish `owlcms/jurybox/break/A stop`.
    - When a referee decision has been made, immediately display the decision on the jury control panel.
        - Subscribe to `owlcms/fop/decision/A`.
        - Reset the decision on `owlcms/fop/resetDecisions/A` .
- Timekeeper
    - When the timekeeper starts the clock, publish `owlcms/clock/A start`.
    - When the timekeeper stop the clock, publish `owlcms/clock/A stop`.
    - When the timekeeper resets the clock to one minute, publish `owlcms/clock/A 60`.
    - When the timekeeper resets the clock to two minutes, publish `owlcms/clock/A 120`.
