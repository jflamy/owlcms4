...
![image-20260717204355622](img/030-AdvancedSystemSettings/image-20260717204355622.png)
...
- Public Results Scoreboard URL 
  - The `publicresults Cloud Application URL` is normally refers to a [Tracker](./170-Tracker.md) program running in the cloud.   It will take the form `wss://scoreboards.fly.dev/ws`  (where scoreboard is the actual name of the application in the cloud).  To create a Tracker in the cloud, you will use the same steps as described on the [Cloud Installation](../020-RunningInTheCloud/010-Fly) page, but you will use the Tracker and Shared Key sections.
...
![10](img/030-AdvancedSystemSettings/10.png)
...
- **Backdoor Access IP**  This grants access without requesting a password, to all the technical official screens and displays.  Setting this value also enables access to two other pages that are normaly hidden
  - the `/simulstion`page to [launch a simulated competition](./160-Simulation).  
...
![image-20260718084214010](img/030-AdvancedSystemSettings/image-20260718084214010.png)
...
- Visual Styling
  ...
  When started from the [Control Panel](../010-RunningLocally/010-LocalControlPanel.md) OWLCMS copies 4 visual themes in the `local/css`directory
  ...
  These files can be edited as explained in the  [Scoreboard Styling](./080-Styles) page
