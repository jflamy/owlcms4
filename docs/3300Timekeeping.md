The timekeeper screen is straightforward.  It allows starting/stopping the time, and resetting the clock to the one-minute or two-minute timer value.

![10](img/3300Timekeeping/10.png)

The timekeeper screen is often displayed on a tablet.  It works on iPhones, iPads and Android phones.

- if using the screen with a laptop, you can use the `,` (comma) key to start the timer, and the `.` (period) key to stop it.  This also means that you can connect a programmable button device to control the clock 
- The `*` key on the numeric keypad acts as a toggle (start if stopped, stop if started).
- The clock can be reset to the one-minute or two-minute timer value using the buttons, or by using the `-` and `=` keys respectively. These values are normally 1:00 and 2:00, and can be configured as described in [Configuration](Configuration#athlete-timer-parameters).
