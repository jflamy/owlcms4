...
| resetMode                 | OWLCMS_RESETMODE          | false             | If true, erase the database completely and recreate the database tables using the current OWLCMS_INITIALDATA (by default, EMPTY_COMPETITION) |
| initialData               | OWLCMS_INITIALDATA        | EMPTY_COMPETITION | Used together with OWLCMS_RESETMODE=true<br />Possible Values: <br />EMPTY_COMPETITION : empty database, ready for competition.<br />LARGEGROUP_DEMO : Insert a full 20-athlete group and typical-sized groups<br />SINGLE_ATHLETE_GROUPS : Insert groups with a single athlete, useful to rehearse breaks and end-of-group situations.<br />BENCHMARK: insert 80+ sessions of 14 athletes running on 4 concurrent platforms for running large stress tests. |
