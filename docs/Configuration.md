...
| (environment only)        | OWLCMS_MDNS               | Announce a LAN hostname before web-server and database startup. Unset defaults to `owlcms` (`owlcms.local`); another value selects a different name. An empty value, `false`, or `off` disables announcement. Announcement is automatically skipped for remote database configurations detected from `JDBC_DATABASE_URL`, `DATABASE_URL`, and `POSTGRES_HOST`. This replaces the persisted `disableMdns` feature switch; no database initialization is needed. |
