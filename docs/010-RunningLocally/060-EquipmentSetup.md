...
In a small club meet, the announcer will often do everything - announce, enter the weight changes, and operate the clock. This can be done on a single laptop, as demonstrated in this [video](./090-Demo1).  
...
<table>
<tr><td><img src='img/060-EquipmentSetup/ElSalvador.jpg'></img></td><td><img src='img/060-EquipmentSetup/ElSalvador_marshall.jpg'></img></td></tr></table>
...
![setup-local-club-minimal.drawio](img/060-EquipmentSetup/setup-local-club-minimal.drawio-4740776.svg)

**IMPORTANT**: a small router is needed so the computers ot tablets can talk to OWLCMS (even the most basic model will work).  If you have good internet access and don't want to deal with that, you can [run in the cloud](../020-RunningInTheCloud/010-Fly) instead.
...
![](img/060-EquipmentSetup/setup-local-club.drawio.svg)
...
![StateCompetition](img/060-EquipmentSetup/StateCompetition.png)
...
![setup-full](img/060-EquipmentSetup/setup-full-5433871.svg)
...
- Apple iPhones and iPads are ok as [mobile refereeing devices](../060-Refereeing/010-Refereeing#mobile-device-refereeing).   Display features such as the Scoreboard and the refereeing displays (attempt board, athlete-facing decisions) also work.

## Sound Requirements

By default, only the athlete-facing decision display emits sound.  See this [page](../070-ScoreboardsAndResults/010-Displays#display-settings) for controlling the sound parameters for the various displays if you need to enable it on another display.  You should normally enable sound only on one display per room, multiple sources are confusing.

If the equipment used for display has no speakers, you can get the main computer to generate the sounds.   See [these explanations.](../030-PreCompetitionSetup/060-Preparation#associating-an-audio-output-with-a-platform)
...
- You are using a local router, but there is no Ethernet access to the Internet.  There are 3 options:
  - If there is Wi-Fi at the facility, you can wire your owlcms and OBS computers to the local router and also connect them to the facility's Wi-Fi. There is nothing required for macOS or Raspberry Pi.  For Windows, see [Using Both Ethernet and Wi-Fi](../090-Video/030-WiFiPlusEthernet)
  - There is no Wi-Fi, but you can use a phone as a hotspot. You would use the same approach as above, see [Using Both Ethernet and Wi-Fi](../090-Video/030-WiFiPlusEthernet)
